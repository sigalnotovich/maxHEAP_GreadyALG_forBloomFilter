...
def max_flow(C, s, t, num_trx_in_current_block):
    ...
    #from trx1 to trxN :
        #if F[trx1][t] == 3:
            #print (indx of trx1)
    return sum(F[s][i] for i in range(n)) , unique_bits_trx
...
C_new, num_trx_in_current_block, arraylen = new_matrix(data, number_of_trx_in_new_block)
# print_graph(C_new)

source = 0  # A
sink = arraylen - 1 # F
max_flow_value,unique_bits_trx = max_flow(C_new, source, sink, num_trx_in_current_block)
...
###data = pd.read_csv(csv_file)
#def make_new_file(unique_bits_trx,data):
